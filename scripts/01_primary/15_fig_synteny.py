...
rows=list(S.items())
H=2.2
fig,ax=plt.subplots(figsize=(7.4,1.15*len(rows)+1.4))
for i,(name,feats) in enumerate(rows):
    ...
    ax.text(0.0,y+0.92,name,ha='left',va='bottom',fontsize=6.4,fontweight='bold')
    ...
    for s,e,st,nm in feats:
        ...
        big=(c=='#c1272d')
        off=-0.34 if k%2==0 else -0.78
        ax.text(x0+w/2,y+((0.40 if not any(('transposon' in f[3] or f[3]=='mobile element') and f[0]<=s and e<=f[1] for f in feats) else 0.86) if big else off),nm,ha='center',va=('bottom' if big else 'top'),
                fontsize=(5.4 if big else 3.9),color=('#c1272d' if big else '#444444'),
...
ax.legend(handles=leg,fontsize=5.8,frameon=False,loc='lower left',ncol=2,bbox_to_anchor=(0.03,-0.02))
plt.savefig('figures/Figure4_synteny.png',dpi=600,bbox_inches='tight')
plt.savefig('figures/Figure4_synteny.pdf',bbox_inches='tight')
print('ok')
